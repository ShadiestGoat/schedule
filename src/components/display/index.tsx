...
const DisplayComp:FunctionComponent<{current:fullClassInfo, next:fullClassInfo, after?:fullClassInfo}> = ({ current, next, after }) => {
    const [vert, setVert] = useState<boolean>(getV())
    useGlobalListener("resize", () => {
        setVert(getV())
    })
    return <div>
        <div class={vert ? "col" : "row"} style={{width: "60%", margin: "auto auto", marginTop:vert ? "0px" : "27vh", height: vert ? "95vh" : ""}}>
            <div class="col">
                <h1> Now </h1>
                <Bar />
